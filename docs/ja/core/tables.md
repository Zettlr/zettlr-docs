...
はるかに良いですよね？Zettlrのテーブルエディタは、文書中に見つけたすべてのMarkdownの表を、編集可能なセルを持ったHTML形式のテーブルに変換して、元のMarkdownの代わりに表示します。これだとMarkdownのソースは完全に隠れてしまいますが、この場合は、このアプローチの方がWYSIWYMアプローチよりも優れています。
