...
::: note
Since the split view is more a concept to organize documents, and less a specific "thing," terms can get confusing. The context menu entry is called "leaf" because the split view is organized using a tree structure. What we call "Split View" usually means the entirety of all split elements (that is, the entire tree). You can call individual split elements "panes" instead of "leafs," since that is how we call them internally.
:::
